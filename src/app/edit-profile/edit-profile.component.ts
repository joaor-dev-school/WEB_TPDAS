...
import {
  EditProfileStateDataModel,
  initialEditProfileStateData,
} from "../shared/edit-profile/models/edit-profile-state-data.model";
...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userProfileForm = this.fb.group({
      ...
      timeInit: this.fb.control(""),
      timeEnd: this.fb.control(""),
    ...
    this.stateData = initialEditProfileStateData(
      this.router,
      this.authService,
      this.alertService,
  ...
  async checkPassword(): Promise<void> {
    this.state = await this.state.checkPassword(
  ...
  insertPref(): void {
    this.state = this.state.changePreferences(
      this.userProfileForm.get("timeInit").value
    );
  }

  async changeUser(): Promise<void> {
    this.state = await this.state.changeUser(
      this.userProfileForm.get("name").value
    );
  }

  async changePassword(): Promise<void> {
    this.state = await this.state.changePass(
      this.userProfileForm.get("password").value
    );
  }
}
